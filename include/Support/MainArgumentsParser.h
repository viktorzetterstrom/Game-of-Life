...
public:
	/**
	  ...
	  *@details When arguments are given by the command line, it goes through those arguments and excecutes its function.
	  *@param argv[] the arguments provided by the user in Command-line
	  *@param length Number of arguments
	  *@test Given valid arguments, after finding its value, it should return the reference to an ApplicationValues struct ccorresponding the right argument. If the argument is -h, it should excecute the app directly. 
...
private:
	...
	*@details Looking for the option by searching through the passed characters
	...
	*@details iterates through the options and returns the passed option if found
	*@param begin Start of characters
	*@param end End of characters
	*@param option The value of the option that should be searched for
	*@test Should return a pointer to the value of the given option, if not it returns 0
	*/
